...
# `# Create a background SSH tunnel` line. ssh-keyscan runs before the
# retry-protected tunnel, so a transient engine-host blip here must retry too
# rather than fail the whole job (issues #241, #243, #772).
...
# / KEYSCAN_STDERR_MSG. On "success" it prints a fake host key to stdout (which
# the block appends to known_hosts); on failure it prints nothing to stdout, as
# the real ssh-keyscan does when it cannot read any keys.
...
# A fresh HOME per run keeps ~/.ssh/known_hosts writes isolated between tests.
run_keyscan() {
...
# --- Success on first attempt: silent, one keyscan call, no retry warning ------
...
out=$(KEYSCAN_RC=1 KEYSCAN_STDERR_MSG="getaddrinfo: Name or service not known" KEYSCAN_SUCCEED_ON=2 run_keyscan)
rc=$?
attempts=$(cat "$SCRATCH/keyscan_attempts")
if [ "$rc" -ne 0 ]; then
...
# --- Persistent failure through all attempts: retries then errors and exits ---
out=$(KEYSCAN_RC=1 KEYSCAN_STDERR_MSG="getaddrinfo: Name or service not known" run_keyscan)
rc=$?
attempts=$(cat "$SCRATCH/keyscan_attempts")
if [ "$rc" -eq 0 ]; then
...
elif ! printf '%s' "$out" | grep -q "getaddrinfo: Name or service not known"; then
    _fail "keyscan persistent: should surface the captured stderr in diagnostics" "$out"
...
# --- Default budget is generous (issue #772): 5 attempts, 15s wait ------------
# Regression guard mirroring the tunnel-budget guard above, so a future edit
# doesn't quietly tighten the keyscan retry loop back to a single shot.

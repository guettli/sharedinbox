...
# bare `Process completed with exit code 1.` with no clue. Pin the new
# retry+diagnostics behavior. See issue #174.
_keyscan_snippet=$(awk '/^# Add remote host to known_hosts/{p=1} /^# Create a background SSH tunnel/{p=0} p' "$SCRIPT")
...
# --- keyscan fails 3 times: surfaces stderr in diagnostics, exits non-zero -----
...
if [ "$rc" -eq 0 ]; then
    ...
    _fail "keyscan fail: should mention the attempt count in the error" "$out"

...
class MailboxRepositoryIntegrationTest {
    ...
    @Test
    fun syncMailboxes_inboxPresent() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
            mailboxRepo.syncMailboxes(account.id).getOrThrow()
    ...
    @Test
    fun syncMailboxes_twoAccountsAreIndependent() =
        runBlocking {
    ...
    @Test
    fun syncMailboxes_incrementalSyncIdempotent() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun removeAccount_cascadesMailboxes() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
            mailboxRepo.syncMailboxes(account.id).getOrThrow()
    ...
    @Test
    fun createMailbox_appearsInDbAndServer() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
            mailboxRepo.syncMailboxes(account.id).getOrThrow()
    ...
    @Test
    fun createMailbox_logsSuccessInSyncLog() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun renameMailbox_updatesNameLocally() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
            ...
            val mailboxes = mailboxRepo.observeMailboxes(account.id).first()
            assertTrue(
                mailboxes.any { it.id == created.id && it.name == "Renamed" },
    ...
    @Test
    fun renameMailbox_logsSuccessInSyncLog() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun deleteMailbox_removedFromDb() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun deleteMailbox_logsSuccessInSyncLog() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun deleteMailbox_systemMailbox_logsConflict() =
        runBlocking {
    ...
    @Test
    fun syncMailboxes_logsInSyncLog() =
        runBlocking {
            val account = accountRepo.addAccount(baseUrl, userA, passA).getOrThrow()
            mailboxRepo.syncMailboxes(account.id).getOrThrow()

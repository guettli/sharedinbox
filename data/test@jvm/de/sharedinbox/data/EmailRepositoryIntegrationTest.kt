...
class EmailRepositoryIntegrationTest {
    ...
    fun setUp() = runBlocking {
        ...
        emailRepo = EmailRepositoryImpl(db, tokenStore, syncLogRepo, mailboxRepo)
    ...
    @Test
    fun syncEmails_populatesDb() = runBlocking {
    ...
    @Test
    fun observeEmails_emitsFlow() = runBlocking {
    ...
    @Test
    fun syncEmails_stateTokenSaved() = runBlocking {
        emailRepo.syncEmails(aliceId, aliceInboxId).getOrThrow()
    ...
    @Test
    fun syncEmails_incrementalSync_idempotent() = runBlocking {
    ...
    @Test
    fun syncEmails_twoAccounts_independent() = runBlocking {
    ...
    fun getEmail_usesLocalCache() = runBlocking {
        ...
        val bodyText = secondCall.textBody.firstOrNull()
            ?.partId?.let { secondCall.bodyValues[it]?.value }
    ...
    @Test
    fun setKeyword_marksEmailAsSeen() = runBlocking {
        createTestEmail("Keyword test")
        emailRepo.syncEmails(aliceId, aliceInboxId).getOrThrow()

        val emailId = emailRepo.observeEmails(aliceId, aliceInboxId).first()
            .first { it.subject == "Keyword test" }.id
        ...
        assertTrue(updated.keywords.contains(EmailKeyword.SEEN),
            "Expected \$seen in keywords, got: ${updated.keywords}")
    }

    @Test
    fun setKeyword_removesKeyword() = runBlocking {
        createTestEmail("Unflag test")
        emailRepo.syncEmails(aliceId, aliceInboxId).getOrThrow()
        val emailId = emailRepo.observeEmails(aliceId, aliceInboxId).first()
            .first { it.subject == "Unflag test" }.id
        ...
        val updated = db.emailHeaderQueries.selectEmailById(aliceId, emailId).executeAsOneOrNull()
        assertNotNull(updated)
        assertTrue(!updated.keywords.contains(EmailKeyword.FLAGGED),
            "Expected \$flagged to be removed, got: ${updated.keywords}")
    }

    @Test
    fun moveEmail_updatesMailboxId() = runBlocking {
        createTestEmail("Move test")
        emailRepo.syncEmails(aliceId, aliceInboxId).getOrThrow()

        val emailId = emailRepo.observeEmails(aliceId, aliceInboxId).first()
            .first { it.subject == "Move test" }.id

        val mailboxes = mailboxRepo.observeMailboxes(aliceId).first()
        val sentMailbox = mailboxes.firstOrNull { it.role == MailboxRole.SENT }
    ...
    @Test
    fun deleteEmail_movesToTrash() = runBlocking {
        createTestEmail("Delete test")
        emailRepo.syncEmails(aliceId, aliceInboxId).getOrThrow()

        val emailId = emailRepo.observeEmails(aliceId, aliceInboxId).first()
            .first { it.subject == "Delete test" }.id

        val trashMailbox = mailboxRepo.observeMailboxes(aliceId).first()
        ...
        if (trashMailbox != null) {
            ...
            assertEquals(trashMailbox.id, inTrash.mailbox_id,
    ...
    @Test
    fun sendEmail_succeeds() = runBlocking {
    ...
    private suspend fun createTestEmail(
        ...
        val result = aliceApiClient.emailSet(aliceJmapId, buildJsonObject {
            put("create", buildJsonObject {
                put("new1", buildJsonObject {
                    ...
                    put("textBody", buildJsonArray {
                        add(buildJsonObject { put("partId", "body1"); put("type", "text/plain") })
                    })
                })
            })
        })

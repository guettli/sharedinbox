...
class AccountRepositoryIntegrationTest {
    ...
    fun setUp() {
        ...
        val tokenStore = FileTokenStore(createTempFile(prefix = "sharedinbox-test-creds-", suffix = ".json").also { it.toFile().deleteOnExit() })
        repo = AccountRepositoryImpl(db, tokenStore, SessionRepositoryImpl())
    }

    @Test
    fun addAccount_returnsAccountWithCorrectFields() = runBlocking {
    ...
    @Test
    fun addAccount_persistsToDb() = runBlocking {
    ...
    @Test
    fun addTwoAccounts_bothInDb() = runBlocking {
        repo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun removeAccount_deletesFromDb() = runBlocking {
        val account = repo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun removeOneAccount_otherAccountRemains() = runBlocking {
    ...
    @Test
    fun addAccount_credentialsSaved() = runBlocking {
        val account = repo.addAccount(baseUrl, userA, passA).getOrThrow()
    ...
    @Test
    fun removeAccount_clearsCredentials() = runBlocking {
        val account = repo.addAccount(baseUrl, userA, passA).getOrThrow()
        repo.removeAccount(account.id)
    ...
    @Test
    fun addAccount_badCredentials_returnsFailure() = runBlocking {

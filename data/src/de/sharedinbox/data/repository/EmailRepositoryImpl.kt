...
class EmailRepositoryImpl(
    ...
    override fun observeEmails(accountId: String, mailboxId: String): Flow<List<Email>> =
        db.emailHeaderQueries.selectEmailsByMailbox(accountId, mailboxId)
            .asFlow()
    ...
    override suspend fun syncEmails(accountId: String, mailboxId: String): Result<Unit> =
        runCatching {
            try {
    ...
    private suspend fun fullSyncEmails(
        ...
        val queryResult = apiClient.queryEmails(jmapAccountId, mailboxId)
    ...
    override suspend fun getEmail(accountId: String, emailId: String): Result<Email> =
        runCatching {
            val header = db.emailHeaderQueries.selectEmailById(accountId, emailId)
                .executeAsOneOrNull()
                ?: error("Email $emailId not found in local DB — sync first")

            val cachedBody = db.emailBodyQueries
            ...
            withApiClient(accountId) { apiClient, account ->
                ...
                val textContent = bodyOnly.textBody.firstOrNull()
                    ?.partId?.let { bodyOnly.bodyValues[it]?.value }
                val htmlContent = bodyOnly.htmlBody.firstOrNull()
                    ?.partId?.let { bodyOnly.bodyValues[it]?.value }
    ...
    override suspend fun setKeyword(
        ...
        withApiClient(accountId) { apiClient, account ->
            val patch = buildJsonObject {
                if (set) put("keywords/$keyword", true)
                else put("keywords/$keyword", JsonNull)
            }
            val result = apiClient.emailSet(account.jmap_account_id, buildJsonObject {
    ...
    override suspend fun sendEmail(accountId: String, draft: EmailDraft): Result<Unit> =
        runCatching {
            withApiClient(accountId) { apiClient, account ->
                ...
                val identity = runCatching {
                    apiClient.getIdentities(account.jmap_account_id).list
                        .firstOrNull { !it.email.isNullOrBlank() }
                }.getOrNull()

                val mailboxes = db.mailboxQueries.selectMailboxesByAccount(accountId).executeAsList()
                val draftsMailbox = mailboxes.firstOrNull { it.role == "\$draft" || it.role == "drafts" }
                    ?: mailboxes.firstOrNull()
                    ?: error("No mailbox found for account $accountId — sync mailboxes first")

                if (identity != null) {
                    // Full JMAP submission path
                    val createResult = apiClient.emailSet(account.jmap_account_id, buildJsonObject {
                        put("create", buildJsonObject {
                            put("draft1", buildJsonObject {
                                put("mailboxIds", buildJsonObject { put(draftsMailbox.id, true) })
                                put("from", json.encodeToJsonElement(
                                    kotlinx.serialization.serializer<List<EmailAddress>>(), listOf(draft.from)))
                                put("to", json.encodeToJsonElement(
                                    kotlinx.serialization.serializer<List<EmailAddress>>(), draft.to))
                                if (draft.cc.isNotEmpty()) {
                                    put("cc", json.encodeToJsonElement(
                                        kotlinx.serialization.serializer<List<EmailAddress>>(), draft.cc))
                                }
                    ...
                    if (createResult.notCreated.isNotEmpty()) {
                        val detail = createResult.notCreated.entries.joinToString { (_, e) ->
                            "${e.type}: ${e.description ?: "no description"}"
                        }
                        syncLog.log(
                            accountId = accountId,
                    ...
                    val emailId = createResult.created["draft1"]
                        ?.get("id")
                        ?.let { (it as? JsonPrimitive)?.content }
                        ?: error("Server did not return created email ID")

                    val submissionResult = apiClient.emailSubmissionSet(
                        account.jmap_account_id, identity.id, emailId,
                    )
                    if (submissionResult.notCreated.isNotEmpty()) {
                        val detail = submissionResult.notCreated.entries.joinToString { (_, e) ->
                            "${e.type}: ${e.description ?: "no description"}"
                        }
                        syncLog.log(
                            accountId = accountId,
    ...
    override suspend fun archiveEmail(accountId: String, emailId: String): Result<Unit> =
        runCatching {
            val archiveId = getOrCreateMailboxId(accountId, MailboxRole.ARCHIVE, "Archive")
            moveEmail(accountId, emailId, archiveId).getOrThrow()
        }

    override suspend fun markAsSpam(accountId: String, emailId: String): Result<Unit> =
        runCatching {
    ...
    override suspend fun searchEmails(accountId: String, query: String): Result<List<Email>> =
        runCatching {
    ...
    override suspend fun getAttachments(
        accountId: String,
        emailId: String,
    ): Result<List<EmailBodyPart>> = runCatching {
        withApiClient(accountId) { apiClient, account ->
    ...
    private fun upsertEmailHeader(accountId: String, email: Email) {
    ...
    private fun decodeKeywords(encoded: String): List<String> =
        if (encoded.isBlank() || encoded == "[]") emptyList()
        else runCatching { json.decodeFromString<List<String>>(encoded) }.getOrDefault(emptyList())

    private fun de.sharedinbox.data.db.Email_header.toDomain(): Email = Email(
        id = id,
        blobId = blob_id,
        threadId = thread_id,
    ...
    private fun Email.withBody(body: de.sharedinbox.data.db.Email_body): Email =
        withBody(body.text_body, body.html_body)

    private fun Email.withBody(textContent: String?, htmlContent: String?): Email = copy(
        bodyValues = buildMap {
            textContent?.let { put("text1", EmailBodyValue(it)) }
            htmlContent?.let { put("html1", EmailBodyValue(it)) }
        },
        textBody = if (textContent != null)
            listOf(EmailBodyPart(partId = "text1", type = "text/plain")) else emptyList(),
        htmlBody = if (htmlContent != null)
            listOf(EmailBodyPart(partId = "html1", type = "text/html")) else emptyList(),
    )
}

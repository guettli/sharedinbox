        ...
        runCatching {
            withApiClient(accountId) { apiClient, account ->
                ...
                if (result.notCreated.isNotEmpty()) {
                    val detail =
                        result.notCreated.entries.joinToString { (_, e) ->
                ...
                val serverId =
                    (createdObj["id"] as? JsonPrimitive)?.content
            ...
            withApiClient(accountId) { apiClient, account ->
                val result =
                    apiClient.mailboxSet(
                        account.jmap_account_id,
                        buildJsonObject {
                            put("destroy", kotlinx.serialization.json.buildJsonArray { add(JsonPrimitive(mailboxId)) })
                        },
                    )
...
private fun de.sharedinbox.data.db.Mailbox.toDomain() =
    Mailbox(
        id = id,
        name = name,
        parentId = parent_id,

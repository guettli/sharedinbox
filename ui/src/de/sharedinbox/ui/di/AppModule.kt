...
val dataModule = module {
    ...
    single<EmailRepository> { EmailRepositoryImpl(get(), get(), get(), get()) }
        ...
        AccountSyncManager(
            ...
            onStateChange = { accountId, changedTypes ->
                ...
                if ("Email" in changedTypes) {
                    val inboxId = mailboxRepo.observeMailboxes(accountId).first()
                        .firstOrNull { it.role == MailboxRole.INBOX }?.id
                    if (inboxId != null) {
